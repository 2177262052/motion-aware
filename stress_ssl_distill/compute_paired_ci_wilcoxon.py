...
def parse_named_path(value: str) -> tuple[str, Path]:
    if "=" not in value:
        raise ValueError(f"Expected NAME=CSV, got: {value}")
...
def wilcoxon_p(delta: np.ndarray, direction: str) -> tuple[float, float]:
    try:
        from scipy.stats import wilcoxon
    except Exception as exc:
        print(f"warning_scipy_unavailable wilcoxon=nan reason={exc}")
        return float("nan"), float("nan")
...
def summarize_pair(
    ...
    signed_ci_low, signed_ci_high = bootstrap_ci(signed_better, n_boot=n_boot, seed=seed)
    ...
    summary = {
        ...
        "signed_candidate_better_ci_low": signed_ci_low,
        "signed_candidate_better_ci_high": signed_ci_high,
        "candidate_wins": wins,
...
def write_markdown(path: Path, summary: pd.DataFrame) -> None:
    lines = [
        "# Paired Bootstrap CI and Wilcoxon Tests",
        "",
        "Unit of analysis: held-out LOSO subject. Delta is candidate minus reference.",
        "",
        "| Dataset | Comparison | Metric | n | Ref mean | Cand mean | Delta | 95% bootstrap CI | W/L/T | Wilcoxon p (two-sided) | Wilcoxon p (candidate better) |",
        "|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in summary.itertuples(index=False):
        lines.append(
            f"| {row.dataset} | {row.comparison} | {row.metric} | {int(row.n)} | "
            f"{row.reference_mean:.4f} | {row.candidate_mean:.4f} | {row.delta_mean:+.4f} | "
            f"[{row.delta_bootstrap_ci_low:+.4f}, {row.delta_bootstrap_ci_high:+.4f}] | "
            f"{int(row.candidate_wins)}/{int(row.candidate_losses)}/{int(row.ties)} | "
            f"{row.wilcoxon_p_two_sided:.4g} | {row.wilcoxon_p_candidate_better:.4g} |"
        )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def main() -> None:
    parser = argparse.ArgumentParser(description="Compute paired subject-level bootstrap CIs and Wilcoxon tests.")
    ...
    parser.add_argument("--metric", action="append", choices=sorted(METRIC_DIRECTIONS), default=None)
    ...
    prefix = args.prefix or f"{args.dataset}_paired_statistics"
    ...
    deltas_path = args.output_dir / f"{prefix}_deltas.csv"
