...
class ScaledMotionFiLM(nn.Module):
    """Historical motion gate with a learned global strength per FiLM block.

    This reproduces checkpoints containing keys like:
        motion_gates.0.scale_logit

    Formula:
        scale = sigmoid(scale_logit)
        y = x * (1 + scale * tanh(gamma)) + scale * beta
    """
...
from .train_wesad_watch import main  # noqa: E402
