...
def discover_manifests(manifests_dir: Path, subjects: Iterable[str] | None, dataset_kind: str = "wesad") -> list[tuple[str, Path]]:
    requested = {subject.strip() for subject in subjects or [] if subject.strip()}
    manifests: list[tuple[str, Path]] = []
    if dataset_kind != "wesad":
        raise ValueError("The release wrapper supports WESAD only.")
    prefix = "wesad"
...
def main() -> None:
    ...
    csv_path = output_dir / "wesad_watch_loso_results.csv"
    summary_path = output_dir / "wesad_watch_loso_summary.txt"
