...
def discover_manifests(manifests_dir: Path, subjects: list[str] | None, dataset_kind: str) -> list[tuple[str, Path]]:
    ...
    if dataset_kind not in {"galaxy", "wesad"}:
        raise ValueError("The release ablation wrapper supports Galaxy and WESAD only.")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Run watch-input LOSO ablations for Galaxy or WESAD.")
    ...
    for subject, manifest_path in manifests:
        ...
        if not args.skip_watch_only:
            ...
            watch_command.extend(["--wavelet-weight", str(args.watch_wavelet_weight)])
            if args.pin_memory:
        ...
        if not args.skip_privileged:
            ...
            deploy_command.extend(["--cross-confidence-min-weight", str(args.cross_confidence_min_weight)])
            if args.pin_memory:
