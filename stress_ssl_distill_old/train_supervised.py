...
def main() -> None:
    parser = argparse.ArgumentParser(description="Supervised baseline on the STRESS protocol.")
    ...
    for epoch in range(args.epochs):
        model.train()
        total_loss = 0.0
        progress = tqdm(train_loader, desc=f"supervised epoch {epoch + 1}/{args.epochs}", leave=True)
    ...
    print(f"Saved supervised baseline checkpoint to {args.save_path}")
