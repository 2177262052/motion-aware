...
from .run_wesad_watch_loso_eval import (
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run WESAD watch-only LOSO using the historical scale_logit motion FiLM."
    )
    ...
    csv_path = output_dir / f"{output_prefix}_watch_scaled_motion_loso_results.csv"
    summary_path = output_dir / f"{output_prefix}_watch_scaled_motion_loso_summary.txt"
    logs_dir.mkdir(parents=True, exist_ok=True)
    ckpt_dir.mkdir(parents=True, exist_ok=True)

    title = f"{args.dataset_kind}_watch_only_{args.model_type}_{args.watch_enhancement}_scaled_motion"
    ...
    print(f"Saved WESAD scaled-motion watch LOSO CSV to {csv_path}")
    print(f"Saved WESAD scaled-motion watch LOSO summary to {summary_path}")
