...
def discover_manifests(manifests_dir: Path, subjects: Iterable[str] | None) -> list[tuple[str, Path]]:
    ...
    for path in sorted(manifests_dir.glob("wesad_*_loso_val.csv")):
        subject = path.stem.replace("wesad_", "").replace("_loso_val", "")
...
def parse_last_metrics(output: str) -> dict[str, float]:
    groups = None
    for item in TINY_PATTERN.finditer(output):
        groups = item.groupdict()
    if groups is None:
        raise ValueError("Could not parse WESAD tiny distillation metrics from command output.")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run WESAD LOSO tiny-student distillation from per-subject watch teachers.")
    ...
    csv_path = output_dir / "wesad_tiny_loso_results.csv"
    summary_path = output_dir / "wesad_tiny_loso_summary.txt"
    logs_dir.mkdir(parents=True, exist_ok=True)
    ckpt_dir.mkdir(parents=True, exist_ok=True)

    title = f"wesad_tiny_from_{args.teacher_kind}"
    ...
    for subject, manifest_path in manifests:
        ...
        tiny_command = [
            sys.executable,
            "-m",
            "stress_ssl_distill.train_wesad_tiny_distill",
            "--manifest",
            str(manifest_path),
            "--dataset-root",
            ...
            "--seed",
            str(args.seed),
            "--teacher-watch-only-model-type",
            args.teacher_watch_only_model_type,
            "--teacher-watch-backbone",
            args.teacher_watch_backbone,
            "--watch-model-dim",
        ...
        tiny_metrics = parse_last_metrics(tiny_output)
    ...
    print(f"Saved WESAD tiny LOSO CSV to {csv_path}")
    print(f"Saved WESAD tiny LOSO summary to {summary_path}")
