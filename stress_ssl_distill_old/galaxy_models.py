...
class ResNet34WatchNet(nn.Module):
    ...
    def forward(
        ...
        logits = self.classifier(embedding)
        contrastive = F.normalize(self.contrastive_head(embedding), dim=1)
...
class WaveletGuidedWatchNet(nn.Module):
    ...
    def forward(
        ...
        contrastive = F.normalize(self.contrastive_head(fused), dim=1)
        wavelet_pred = self.wavelet_predictor(out["temporal_embedding"])
        out["embedding"] = fused
        out["logits"] = logits
        out["contrastive"] = contrastive
...
class PrivilegedGalaxyTeacherNet(nn.Module):
    def __init__(
        ...
        use_teacher_fused_classifier: bool = True,
        ...
        self.use_teacher_fused_classifier = use_teacher_fused_classifier
        ...
        self.teacher_fused_classifier = nn.Linear(embed_dim, num_classes) if use_teacher_fused_classifier else None
        if self.num_phenotypes > 0:
            self.phenotype_router_context = nn.Sequential(
                nn.Linear(embed_dim * 3, embed_dim),
                nn.LayerNorm(embed_dim),
                nn.GELU(),
            )
            self.phenotype_router = nn.Linear(embed_dim, self.num_phenotypes)
            self.phenotype_experts = nn.ModuleList(
                [
                    nn.Sequential(
            ...
            self.phenotype_adapter_gate = nn.Sequential(
        ...
        self.watch_projector = (
            nn.Sequential(
                nn.Linear(embed_dim, 160),
                nn.GELU(),
                nn.Linear(160, proj_dim),
            )
    ...
    def forward_watch(
        ...
        if self.use_student_gated_correction:
            ...
            logits = self.watch_classifier(deploy_embedding)
            out["logits"] = logits
        ...
        if return_aux and self.watch_projector is not None:
            out["watch_proj"] = F.normalize(self.watch_projector(embedding), dim=1)
    ...
    def forward(
        ...
        if e4_signal is not None:
            ...
            if return_aux and self.e4_projector is not None:
                out["e4_proj"] = F.normalize(self.e4_projector(e4_embedding), dim=1)
            ...
            watch_specific_features = torch.cat(
                [
                    ...
                    torch.abs(watch_teacher - attended_e4),
                ],
                dim=1,
            )
            ...
            if self.num_phenotypes > 0:
                phenotype_context = self.phenotype_router_context(
                    torch.cat([watch_teacher, teacher_watch_path, teacher_fused_path], dim=1)
                )
                phenotype_logits = self.phenotype_router(phenotype_context)
                phenotype_probs = torch.softmax(phenotype_logits, dim=1)
                expert_input = torch.cat([blended_path, phenotype_context], dim=1)
                phenotype_experts = torch.stack([expert(expert_input) for expert in self.phenotype_experts], dim=1)
                phenotype_adapter = torch.sum(phenotype_probs.unsqueeze(-1) * phenotype_experts, dim=1)
                phenotype_gate = self.phenotype_adapter_gate(torch.cat([teacher_embedding, phenotype_context], dim=1))
                teacher_embedding = teacher_embedding + phenotype_gate * phenotype_adapter
                out["phenotype_logits"] = phenotype_logits
                out["phenotype_probs"] = phenotype_probs
                out["phenotype_adapter"] = phenotype_adapter
            ...
            if return_aux and self.use_student_gated_correction:
                quality_col = self._quality_column(quality)
            ...
            if return_aux and self.teacher_fused_classifier is not None:
                out["teacher_fused_logits"] = self.teacher_fused_classifier(teacher_fused_path)
