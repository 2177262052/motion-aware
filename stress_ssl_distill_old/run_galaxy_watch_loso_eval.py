...
def parse_last_metrics(output: str) -> dict[str, float]:
    ...
    if groups is None:
        raise ValueError("Could not parse watch-only metrics from command output.")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run watch-only LOSO evaluation for a chosen Galaxy watch model.")
    ...
    csv_path = output_dir / "galaxy_watch_loso_results.csv"
    summary_path = output_dir / "galaxy_watch_loso_summary.txt"
    logs_dir.mkdir(parents=True, exist_ok=True)
    ckpt_dir.mkdir(parents=True, exist_ok=True)

    title = f"watch_only_{args.model_type}"
    ...
    print(f"Saved watch LOSO CSV to {csv_path}")
    print(f"Saved watch LOSO summary to {summary_path}")
