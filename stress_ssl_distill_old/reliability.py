...
def reliability_bce_loss(reliability_logit: torch.Tensor, trust: torch.Tensor) -> torch.Tensor:
    target = trust.detach().float().clamp(0.0, 1.0)
    return F.binary_cross_entropy_with_logits(reliability_logit.view_as(target), target)
